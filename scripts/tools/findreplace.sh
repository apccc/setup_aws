...
F=`~/setup_aws/scripts/tools/escapeStringForPcregrep.sh "$FIND"`
R=`~/setup_aws/scripts/tools/escapeStringForPcregrep.sh "$REPLACE"`
...
if [ `/usr/bin/pcregrep -c -M -f $FPCREGREP $FILE` -eq 1 ];then
	if [ `/usr/bin/pcregrep -c -M -f $RPCREGREP $FILE` -lt 1 ];then
	        F=`~/setup_aws/scripts/tools/escapeStringForSed.sh "$FIND"`
		R=`~/setup_aws/scripts/tools/escapeStringForSed.sh "$REPLACE"`
		echo ""
