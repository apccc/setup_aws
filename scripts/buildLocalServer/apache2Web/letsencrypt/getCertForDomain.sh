...
~/setup_aws/scripts/buildLocalServer/apache2Web/letsencrypt/installLetsencrypt.sh
...
#get the cert
sudo -H /root/letsencrypt/letsencrypt-auto certonly --agree-tos --standalone --renew-by-default --text -vvvvvv --email "$ZADMINEMAIL" -d "$ZDOMAIN" 2>&1 | sudo tee -a "${LOGDIRECTORY}/getcert.${ZDOMAIN}.log" > /dev/null
echo " * Check log for more details ${LOGDIRECTORY}/getcert.${ZDOMAIN}.log"

#put the cert in place
~/setup_aws/scripts/buildLocalServer/apache2Web/letsencrypt/updateCertRecordsForDomain.sh "$ZDOMAIN"
