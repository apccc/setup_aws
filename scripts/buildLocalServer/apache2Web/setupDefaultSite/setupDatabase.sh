...
if [[ `$MY "USE ${SYSTEM_DATABASE};SHOW TABLES LIKE 'login_tries';" | tail -n +2 | wc -l` -lt 1 ]];then
  ...
  X=$X'`id` bigint(10) NOT NULL,'
  X=$X'`email` varchar(200) COLLATE utf8_unicode_ci NOT NULL DEFAULT "",'
  X=$X'`ip` varchar(50) COLLATE utf8_unicode_ci NOT NULL DEFAULT "",'
  X=$X'`timestamp` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'
  X=$X') ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;'
  $MY "$X"
...
if [[ `$MY "USE ${SYSTEM_DATABASE};SHOW TABLES LIKE 'whitelisted_ips';" | tail -n +2 | wc -l` -lt 1 ]];then
  ...
  X=$X'`ip` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT "",'
  X=$X'`timestamp` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP'
...
if [[ `$MY "USE ${SYSTEM_DATABASE};SHOW TABLES LIKE 'js';" | tail -n +2 | wc -l` -lt 1 ]];then
  ...
  X=$X'`id` int(10) unsigned NOT NULL,'
  X=$X'`title` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT "",'
  X=$X'`description` text COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT "",'
  X=$X'`script` longtext COLLATE utf8mb4_unicode_ci NULL,'
  X=$X'`ord` int(10) NOT NULL DEFAULT "0",'
  X=$X'`whitelisted_ips` TEXT NULL,'
  X=$X'`lastUpdated` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'
  X=$X') ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;'
...
if [[ `$MY "USE ${SYSTEM_DATABASE};SHOW TABLES LIKE 'documentation';" | tail -n +2 | wc -l` -lt 1 ]];then
  ...
  X=$X'`id` int(10) unsigned NOT NULL,'
  X=$X'`title` varchar(200) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT "",'
  X=$X'`category` varchar(100) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT "",'
  X=$X'`content` longtext COLLATE utf8mb4_unicode_ci NULL,'
...
if [[ `$MY "USE ${SYSTEM_DATABASE};SHOW TABLES LIKE 'sites';" | tail -n +2 | wc -l` -lt 1 ]];then
  ...
  X=$X'`id` int(10) unsigned NOT NULL,'
  X=$X'`subdomain` varchar(100) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT "",'
  X=$X'`aliases` TEXT NULL,'
  ...
  X=$X'`cronjobs` TEXT NULL,'
  X=$X'`rewrites` TEXT NULL,'
  X=$X'`virtualhost_extras` TEXT NULL,'
  X=$X'`dependencies_repositories` TEXT NULL,'
  X=$X'`dependencies_packages` TEXT NULL,'
  X=$X'`dependencies_commands` TEXT NULL,'
  X=$X'`template` VARCHAR(50) NOT NULL DEFAULT "default",'
  X=$X'`database` VARCHAR(100) NOT NULL DEFAULT "",'
  X=$X'`logo` BLOB NULL,'
  X=$X'`favicon` BLOB NULL,'
...
if [[ `$MY "USE ${SYSTEM_DATABASE};SHOW TABLES LIKE 'site_sections';" | tail -n +2 | wc -l` -lt 1 ]];then
  ...
  X=$X'`id` int(10) unsigned NOT NULL,'
  X=$X'`site_id` int(10) unsigned NOT NULL DEFAULT "0",'
  ...
  X=$X'`css_ids` varchar(200) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT "",'
  X=$X'`js_ids` varchar(200) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT "",'
  X=$X'`active` enum("T","F") COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT "T"'
  X=$X') ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;'
  $MY "$X"
...
if [[ `$MY "USE ${SYSTEM_DATABASE};SHOW TABLES LIKE 'site_pages';" | tail -n +2 | wc -l` -lt 1 ]];then
  ...
  X=$X'`code` longtext COLLATE utf8mb4_unicode_ci NULL,'
  X=$X'`active` enum("T","F") COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT "T"'
  X=$X') ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;'
  $MY "$X"
...
if [[ `$MY "USE ${SYSTEM_DATABASE};SHOW TABLES LIKE 'navigation';" | tail -n +2 | wc -l` -lt 1 ]];then
  ...
  X=$X'`id` int(10) unsigned NOT NULL,'
  X=$X'`site_id` int(10) unsigned NOT NULL DEFAULT "0",'
  X=$X'`name` varchar(100) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT "",'
  X=$X'`identifier` varchar(100) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT "",'
  X=$X'`description` text COLLATE utf8mb4_unicode_ci NULL,'
  X=$X'`href` text COLLATE utf8mb4_unicode_ci NULL,'
  X=$X'`content` text COLLATE utf8mb4_unicode_ci NULL,'
  X=$X'`active` enum("T","F") COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT "T"'
  X=$X') ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;'
  $MY "$X"
