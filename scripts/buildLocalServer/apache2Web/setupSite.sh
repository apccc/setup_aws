...
if [ ! -d "$D/htdocs" ];then
  if [ ! -z "${SITETEMPLATE}" ];then
    X=~/setup_aws/settings/server/${SITETEMPLATE}SiteTemplate/htdocs
  fi
...
if [ ! -z "$SITEDATABASE" ] && [ ! -z "${SITETEMPLATE}" ];then
  ...
  if [[ `$MY "SHOW DATABASES LIKE '${SITEDATABASE}';" | tail -n 1 | wc -l` -lt 1 ]];then
    ...
    F=~/setup_aws/settings/server/${SITETEMPLATE}SiteTemplate/databaseSetup.sh
    if [ -f $F ];then
...
if [ ! -z "${SITETEMPLATE}" ];then
  F=~/setup_aws/settings/server/${SITETEMPLATE}SiteTemplate/settingsSetup.sh
  if [ -f $F ];then
...
if [ "$SSL" == "T" ];then
  ...
  if [[ `sudo ls -1 "/root/ssl/$SUBDOMAIN" | wc -l` -lt 1 ]] || [ "$RENEWSSL" == "T" ];then
    ...
    if [ ! -z "$ALIASES" ];then
      X=`echo -e "${ALIASES}\n${SUBDOMAIN}" | tr "\n" ',' | tr ",," "," | sed 's/,$//'`
      ~/setup_aws/scripts/buildLocalServer/apache2Web/letsencrypt/getCertForDomains.sh "$SUBDOMAIN" "$X"
    else
      ~/setup_aws/scripts/buildLocalServer/apache2Web/letsencrypt/getCertForDomain.sh "$SUBDOMAIN"
    fi
  fi
fi
